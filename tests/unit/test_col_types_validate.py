...
def test_col_types_validate():
    """
    Unit test which tests the function col_types validate.
    """
    ...
    result = col_types_validate(dataframe=df, integer_cols=1, text_cols=3)
    ...
    result = col_types_validate(
        dataframe=df, column_schema={"age": "integer", "city": "text", "name": "text"}
    )
    assert isinstance(result, str)
    assert "Check complete" in result
